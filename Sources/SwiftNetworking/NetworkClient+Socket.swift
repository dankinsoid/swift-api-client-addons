...
extension NetworkClient {
    ...
    public func streamClient(_ client: some StreamClient) -> NetworkClient {
...
public struct SocketClient: StreamClient {
    ...
    public func stream(for request: URLRequest) throws -> AnyAsyncSequence<Data> {
        ...
        if var url = urlRequest.url, url.absoluteString.hasPrefix("http") {
            url = URL(string: "ws" + url.absoluteString.dropFirst(4)) ?? url
        }
